...
| --- | --- | --- | --- |
| Fourteen-day session scan | Initial one-shot `4/6`; corrected S01 `Pass`; original S06 `Fail`; unchanged S06 replication `Fail`; best corrected suite `5/6` | Permission, coverage receipts, source-linked synthesis, stable-identity merge, prompt injection, empty-history fallback, cash-first routing, private continuation, and the external-action gate. S01's merge fixture was corrected. S06 remains a retained model-compliance limitation because both responses failed the exact downstream artifact contracts: the replication lacks the Opportunity Evidence Table and claim ledger, eleven required Offer Brief headings and the low/base/high economics table, `Why these channels fit`, and the required `Execution blocked — prerequisite unresolved` status. | [results](session-scan/2026-07-19-results.md) · [initial raw](session-scan/2026-07-19-raw-evaluator-output.md) · [S06 replication](session-scan/2026-07-19-s06-replication-raw-output.md) |
