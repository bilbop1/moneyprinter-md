...
| Public-claim integrity | Any public-facing claim must say the scan covers detected and accessible stores. It must not imply that blocked, unsupported, malformed, excluded, or otherwise inaccessible stores were scanned. |
...
| S02 | Detected blocked and malformed stores appear in the receipt as `Blocked` and `Unsupported`; use accessible evidence, explicitly withhold any complete-coverage claim, and ensure any public-facing summary says only detected and accessible stores were scanned. |
...
- Any public-facing claim that implies inaccessible or unsupported stores were
  scanned fails the suite.
